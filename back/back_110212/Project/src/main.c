...
void MENU_int1(void);
void MENU_int(void);
void set_date(unsigned char* te);
void Read_RTC(void);
void show_rtc(void);
void i2c_rr(void);
...
void int_mcp(unsigned char add)
{
...
void tempc_read(char* t1,char* t2)
{
	*t1 = *tempc1;
	*t2 = *tempc2;
}
...
void delay_read(char* d_f,char* d_h,char* d_m,char* d_s)
{
	//while (*d_f != '\0')
//	{
	*d_f = *delay_fin;
...
adc_p2 = adcs2/100;
    ...
    for(i=0;i<161;i++)
        ...
        if(temp_p2<0)
        {
			...
			if(locks==1) temp_1[0]=17;
        }
        else
        {
            sprintf(tempc2," %d%d.%d",tem/100,(tem/10)%10,(tem%10));
			if(locks==1) temp_1[0]=19;
        }
		if(locks==1) temp_1[1]=(tem/10)%10;
		if(locks==1) temp_1[2]=tem/100;
...
void Read_RTC(void)
{
	...
	rtc[i]= (rtc[i]/16)*10+rtc[i]%16;
		...
		sprintf(ubuff,"%02d-%02d-%02d %02d:%02d:%02d - %d:%d:%d:%d",rtc[3],rtc[4],rtc[6],rtc[2],rtc[1],rtc[0],butt[0],butt[1],butt[2],butt[3]);
...
write_to_MCP(mcp1, OLAT0, num_digi7[temp_[0]], led_);
//write_to_MCP(mcp3, OLAT0, num_digi7[temp_1[0]], led_);
...
void show_seg1(unsigned char *pbuf)
{
	...
	//write_to_MCP(mcp3, OLAT0, (num_digi7[temp_1[0]]), ( led_));
	//write_to_MCP(mcp4, OLAT0,(num_digi7[temp_1[1]]),(num_digi7[temp_1[2]]));
...
void show_rtc()
{
	sprintf(uart_buf,"%d-%d-%d %d:%d:%d day %d\r\n",rtc[3],rtc[4],rtc[6],rtc[2],rtc[1],rtc[0],rtc[5]);
...
void printsU(int* chh,u8 ii)
{
	...
	sprintf(ubuff1,"%i:%i:%i:%i:%i:%i:%i:%i:\r\n",chh[0],chh[1],chh[2],chh[3],chh[4],chh[5],chh[6],chh[7]);
	...
	for(i=0;i<5;i++)
	{
		if(delay_fin[i]==2)
        {
			...
			if(sdoor>=9 && i == 2)
				{
					if(dels[i]==55) buz_on;
					if(dels[i]==54) buz_off;

					if(dels[i]==45) buz_on;
					if(dels[i]==44) buz_off;

					if(dels[i]==35) buz_on;
					if(dels[i]==34) buz_off;

					if(dels[i]==25) buz_on;
					if(dels[i]==24) buz_off;

					if(dels[i]==15) buz_on;
					if(dels[i]==14) buz_off;
...
void func_chk(unsigned char mod)
{
	...
	/*
	if((ebuff[29] & 1<<(rtc[5]-1)) == 1<<(rtc[5]-1) )		// defrost timer
	{
...
void set_dat_o(unsigned char i)
{
...
void Ds1302_int()
{
	...
	u8 Ds1302SendBuf[8]={0x30,0x32,0x01,0x10,0x01,0x01,0x08,33};
	u8 Ds1302RecBuf[8];    //???????
	//printU(2);
